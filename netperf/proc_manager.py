    ...
    def run_process(self, process_type, **kwargs):
        ...
        if process_type == "tcpdump":
            ...
            command = [
                ...
                "-x", os.path.join(kwargs['receiver_dir'],
                                   'logs', "receiver.log")
            ]
            try:
            ...
            try:
                ...
                if kwargs['action'] == "download":
                    kwargs['sftp_client'].get(
                        kwargs['remote_path'], kwargs['local_path'])
                else:
                    kwargs['sftp_client'].put(
                        kwargs['local_path'], kwargs['remote_path'])
                pid = None
            except Exception as e:
            ...
            try:
                ...
                self.logger.error(
                    f"Failed to Decompress {kwargs['file_name']}, {e}")
                raise
            ...
            for pid in kwargs['processes']:
                try:
                    if pid:
                        self.run_process(
                            "kill", pid=pid, ssh_pass=kwargs['ssh_pass'])
                except Exception as e:
            ...
            try:
                ...
                if result.returncode != 0:
                    raise subprocess.CalledProcessError(
                        result.returncode, command)
                self.parser = Parser(
                    result.stdout, kwargs['sender_log_path']).extract_info()
                pid = None
            except subprocess.CalledProcessError as e:
                self.logger.error(
                    f"Failed to parse {kwargs['sender_log_path']}/receiver.log: {e}")
                raise
        elif process_type == "priv":
            executable = os.path.realpath(kwargs['executable'])
            if not executable:
                self.logger.error(
                    "No executable provided for 'priv' operation.")
                return None
            try:
                if kwargs['is_remote']:
                ...
                self.logger.error(
                    f"Failed to capabilities on {executable}: {e}")
            pid = None
        else:

...
class Parser:
    ...
    def extract_info(self):
        ...
        threshold_bps = self.info['average_bitrate']

        cycle_period = 1000
        tol_period_error = 0.04
        tol_packet_loss = 0.1

        self.info['is_throughput'] = "충족" if round(throughput_bps /1000, 2) <= round(threshold_bps, 2) else "충족하지 않음"
        ...
        self.logger.info(f"Throughput(kbps): {self.info['throughput_kbps']:.02f} {threshold_bps}, 결과: {self.info['is_throughput']}")
